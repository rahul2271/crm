    ...
    const data = await DailyEntry.aggregate([
      ...
      ...(Object.keys(subMatch).length ? [{ $match: subMatch }] : []),
      {
        $group: {
          ...
          onlineLeads:         { $sum: { $cond: [{ $eq: ['$entries.consultationType', 'online'] },          '$entries.leadsCount', 0] } },
          hospitalLeads:       { $sum: { $cond: [{ $eq: ['$entries.consultationType', 'hospital'] },        '$entries.leadsCount', 0] } },
          whatsappLeads:       { $sum: { $cond: [{ $eq: ['$entries.consultationType', 'whatsapp'] },        '$entries.leadsCount', 0] } },
          callLeads:           { $sum: { $cond: [{ $eq: ['$entries.consultationType', 'call'] },            '$entries.leadsCount', 0] } },
          websiteBookingLeads: { $sum: { $cond: [{ $eq: ['$entries.consultationType', 'website_booking'] }, '$entries.leadsCount', 0] } },
        },
      },
      { $lookup: { from: 'users', localField: '_id', foreignField: '_id', as: 'user' } },
      { $unwind: '$user' },
      {
        $project: {
          ...
          whatsappLeads:       1,
          callLeads:           1,
          websiteBookingLeads: 1,
          conversionRate:      convRateProjection,
          avgRevenuePerDay:    { $cond: [{ $gt: [{ $size: '$daysWorked' }, 0] }, { $round: [{ $divide: ['$totalRevenue', { $size: '$daysWorked' }] }, 0] }, 0] },
          avgLeadsPerDay:      { $cond: [{ $gt: [{ $size: '$daysWorked' }, 0] }, { $round: [{ $divide: ['$totalLeads',   { $size: '$daysWorked' }] }, 1] }, 0] },
          utilizationRate:     { $round: [{ $multiply: [{ $divide: ['$totalLeads', { $max: ['$totalLeadsGiven', 1] }] }, 100] }, 1] },
        },
      },
      { $sort: { totalRevenue: -1 } },
