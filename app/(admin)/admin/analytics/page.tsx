...
//                           <td className="px-3 py-2.5 text-orange-600">{t.whatsappLeads}</td>
//                           <td className="px-3 py-2.5 text-orange-600">{t.callLeads}</td>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 mb-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
          ...
          {[
            ...
            { label:'State',      key:'state',        opts:[{v:'',l:'All states'},...INDIAN_STATES.map(s=>({v:s,l:s}))]},
            { label:'Age group',  key:'ageGroup',     opts:[{v:'',l:'All ages'},...AGE_GROUPS.map(g=>({v:g,l:`${g} yrs`}))]},
            { label:'Type',       key:'consultationType', opts:[{v:'',l:'All types'},{v:'online',l:'💻 Online Lead'},{v:'hospital',l:'🏥 Hospital Visit'},{v:'whatsapp',l:'💬 WhatsApp'},{v:'call',l:'📞 Phone Call'},{v:'website_booking',l:'🌐 Website Booking'}]},
          ].map(f => (
            <div key={f.key}>
              <label className="text-xs text-gray-500 block mb-1">{f.label}</label>
              <select value={(filters as any)[f.key]} onChange={e => upd(f.key as keyof Filters, e.target.value)}
        ...
        <div className="space-y-5">
          ...
          {consultStats.length > 0 && (
            <div className={`grid gap-4 ${consultStats.length <= 2 ? 'grid-cols-2' : 'grid-cols-2 lg:grid-cols-4'}`}>
              {consultStats.map((c:any) => {
                const opt = CONSULT_OPTIONS.find(o => o.value === c.consultationType)
                const bg  = opt?.value==='online'?'bg-blue-50 border-blue-100':opt?.value==='hospital'?'bg-orange-50 border-orange-100':opt?.value==='whatsapp'?'bg-green-50 border-green-100':opt?.value==='call'?'bg-purple-50 border-purple-100':'bg-pink-50 border-pink-100'
                const rev = opt?.value==='online'?'text-blue-700':opt?.value==='hospital'?'text-orange-700':opt?.value==='whatsapp'?'text-green-700':opt?.value==='call'?'text-purple-700':'text-pink-700'
                return (
                  <div key={c.consultationType} className={`rounded-xl border p-4 flex items-center justify-between ${bg}`}>
                    <div>
                      <p className="font-semibold text-gray-800">{opt?.icon} {opt?.label ?? c.consultationType}</p>
          ...
          <Card title="Telecaller full performance">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    {['Name','Days','Given','Worked','Converted','Conv. rate','Utilization','Revenue','Rev/day','Leads/day','Online','Hospital','WhatsApp','Phone Call','Website Booking'].map(h => (
                      <th key={h} className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wide px-3 py-3 whitespace-nowrap">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {tcStats.length === 0
                    ? <tr><td colSpan={15} className="text-center py-10 text-gray-400">No data</td></tr>
                    : tcStats.map((t:any,i:number) => (
                        <tr key={t._id||i} className="border-b border-gray-50 hover:bg-gray-50">
                          <td className="px-3 py-2.5">
                            <div className="flex items-center gap-2">
                          ...
                          <td className="px-3 py-2.5 text-gray-500 text-xs">{formatCurrency(t.avgRevenuePerDay)}</td>
                          <td className="px-3 py-2.5 text-gray-500">{t.avgLeadsPerDay}</td>
                          <td className="px-3 py-2.5 text-blue-600">{t.onlineLeads ?? 0}</td>
                          <td className="px-3 py-2.5 text-orange-600">{t.hospitalLeads ?? 0}</td>
                          <td className="px-3 py-2.5 text-green-600">{t.whatsappLeads ?? 0}</td>
                          <td className="px-3 py-2.5 text-purple-600">{t.callLeads ?? 0}</td>
                          <td className="px-3 py-2.5 text-pink-600">{t.websiteBookingLeads ?? 0}</td>
                        </tr>
                      ))
                  }
                </tbody>
